...
export default function GalleryManagement() {
  ...
  const categories = [
    ...
    { id: "donation-drive", label: "Drives", icon: <FaHeart />, count: galleryItems.filter(i => i.category === "donation-drive").length },
    { id: "volunteer", label: "Volunteers", icon: <FaUsers />, count: galleryItems.filter(i => i.category === "volunteer").length },
    { id: "recognition", label: "Awards", icon: <FiAward />, count: galleryItems.filter(i => i.category === "recognition").length },
    { id: "motivational", label: "Stories", icon: <FaQuoteLeft />, count: galleryItems.filter(i => i.category === "motivational").length },
    { id: "campaign", label: "Campaigns", icon: <FiCalendar />, count: galleryItems.filter(i => i.category === "campaign").length }
  ];
  ...
  const UploadModal = () => {
    const [uploadData, setUploadData] = useState({
      title: "", description: "", category: "donation-drive", type: "photo",
      date: new Date().toISOString().split('T')[0], location: "", tags: "", featured: false, status: "draft"
    });
      ...
      <div className="modal-overlay" onClick={() => setShowUploadModal(false)}>
        <div className="modal-content upload-modal" onClick={e => e.stopPropagation()}>
          ...
          <div className="modal-header">
            <h3><FiUpload /> Upload New Media</h3>
  ...
  return (
    <div className="admin-gallery-management">
      ...
      <div className={`gallery-view ${viewMode}`}>
        {paginatedItems.map(item => (
          <div key={item.id} className={`gallery-item ${item.type}`}>
            {/* Selection Checkbox */}
            <div className="item-checkbox" onClick={() => handleSelectItem(item.id)}>
              <input 
                type="checkbox" 
            ...
            <div className="quick-actions">
              ...
              <button onClick={() => { setSelectedImage(item); setShowPreviewModal(true); }}>
                <FiEye />
              </button>
              <button onClick={() => { setSelectedImage(item); setShowEditModal(true); }}>
                <FiEdit2 />
              </button>
            </div>
            ...
            {item.type === 'quote' && (
              <div className={`item-quote ${item.bgColor}`}>
                <FaQuoteLeft className="quote-icon" />
                <p className="quote-text">"{item.content?.substring(0, 40)}..."</p>
              </div>
            )}
      ...
      {showEditModal && <EditModal item={selectedImage} onClose={() => { setShowEditModal(false); setSelectedImage(null); }} />}
      {showPreviewModal && <PreviewModal item={selectedImage} onClose={() => { setShowPreviewModal(false); setSelectedImage(null); }} />}
